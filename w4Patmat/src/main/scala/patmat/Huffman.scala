...
object Huffman {
  ...
  def chars(tree: CodeTree): List[Char] = tree match {
    ...
    case Fork(r, l, _, _) => chars(r) ::: chars(l)
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs match {
    case List() => List()
    case head :: tail => Leaf(head._1, head._2) :: makeOrderedLeafList(tail) sortWith ((x, y) => x.weight < y.weight)
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case first :: second :: tail => makeCodeTree(first, second) :: tail sortWith ((x, y) => weight(x) < weight(y))
    case _ => trees
  }
